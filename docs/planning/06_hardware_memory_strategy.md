...
*   **Motherboard**: **X870E** 칩셋 기반 (E-ATX 또는 High-end ATX 권장).
    *   **필수 조건**: Dual GPU(RTX 5080 + RTX 3090) 장착 시 물리적 간섭이 없는 **넓은 PCIe 슬롯 간격(4-slot spacing)** 또는 최하단 x16 슬롯 지원 모델.
...
| :--- | :--- | :--- | :--- | :--- |
| **GPU 0 (Main)** | **RTX 5080** | **16GB** | **Main Brain (LLM)** | GDDR7 초고속 메모리 활용. **ExLlamaV2** 기반 고속 추론. Main LLM 전용으로 VRAM 효율 극대화. |
| **GPU 1 (Sub)** | **RTX 3090** | **24GB** | **Vision AI**, **TTS** (GPT-SoVITS), **STT**, **3D Rendering** | 넉넉한 VRAM으로 Vision AI, 고품질 TTS 및 렌더링 전담. |
...
-   **할당 전략**:
    -   **GPU 0 (RTX 5080)**: Main LLM (Gemma 4 12B 4-bit) 전용
    -   **GPU 1 (RTX 3090)**: Vision AI(Eye1/Eye2 정량화) + TTS(GPT-SoVITS) + STT + SDXL + 3D Rendering + Hardware Monitor
...
*   **Target Model**: **Gemma 4 12B Unified** (Instruct tuned: `google/gemma-4-12B-it`).
*   **Quantization**: **4-bit GGUF/EXL2**.
    *   *이유*: 4-bit(약 7~8GB)로 RTX 5080 16GB에 여유 있게 상주 가능. 메모리 오프로드 없이 안정적 추론.
    *   *특징*: Gemma 4 12B는 encoder-free unified 아키텍처로 텍스트·이미지·오디오를 네이티브 처리. Vision AI는 RTX 3090에서 정량화 후 텍스트로 전달하여 GPU 0 부담을 최소화.
*   **Context Window**: 8k ~ 32k 실사용 (최대 256K 지원). Vision AI를 RTX 3090으로 분리하여 컨텍스트 여유 확보.

### 2.2. Sub/Emergency Model
...
3.  **Inference (생각)**:
    ...
    *   *참고*: Vision AI 처리는 RTX 3090에서 독립적으로 수행되며, 결과만 LLM에 전달됨.
4.  **Output & Archive (발화 및 기억)**:
    *   답변을 TTS로 출력 및 **RAM(L1)**에 저장.
    *   대화가 길어지면 L1 데이터를 요약하여 L2(SSD)로 이동시키고 L1 비우기 (**Context Sliding**).

### 3.3. 설계 원칙 (Design Principles)

이 메모리 전략의 핵심 결정 사항을 명확히 한다.

*   **"기억" ≠ "KV Cache"**:
    *   기억은 **텍스트/임베딩**으로 저장한다 (L1=RAM, L2=SSD, L3=HDD).
    *   **KV Cache를 RAM/HDD로 분할 저장하지 않는다.** KV Cache는 추론 중에만 쓰는
        휘발성 상태이고, 디스크 페이징 비용이 추론보다 비싸 의미가 없다 (llama.cpp도 미지원).
    *   "단기=RAM / 장기=HDD" 직관은 **KV가 아니라 텍스트 저장소 계층**에 적용한다.
*   **매 턴 새 컨텍스트 (Stateless)**:
    *   하나의 긴 컨텍스트를 누적하지 않고, **매 턴 고정 크기 컨텍스트를 새로 조립**한다.
    *   이점: ① VRAM 사용량이 상수로 유지(16GB에서 예측 가능) ② lost-in-the-middle 회피로
        응답 품질 유지 ③ prefill 연산 감소.
    *   단, "새 컨텍스트 = 기억상실"이 아니다. **단기기억(최근 N턴 원문) + 장기기억(검색된 사실)**
        으로 컨텍스트를 채워서 연다. 모델은 매번 백지로 시작하므로, **기억할 내용을 매 턴 프롬프트에
        다시 넣어주는 것**이 곧 "기억"이다.
*   **역할 분리**:
    *   **말투/페르소나** → 파인튜닝(LoRA)이 담당.
    *   **사실 기억/개인화** → 메모리 시스템(검색)이 담당. 점진적 재학습은 하지 않는다.
*   **설계의 실제 일거리**(모델 바깥 로직):
    1. consolidation: 어떤 대화를 언제 요약해 L2/L3로 넘길까.
    2. 검색 랭킹: 관련도 + 최신성 + 중요도로 끌어올 기억 선정, 중복 제거.
    3. 토큰 예산 배분: 단기 vs 장기 비중.
*   **참고 선행 사례**: MemGPT/Letta, Generative Agents — 동일한 RAG + 요약 메모리 패턴.

> 구현 시점: 정체성 파인튜닝 완료 후 프로토타입 착수 예정
> (sqlite 기억 저장 + 임베딩 검색 + llama.cpp 프롬프트 조립 + 대화 루프).
