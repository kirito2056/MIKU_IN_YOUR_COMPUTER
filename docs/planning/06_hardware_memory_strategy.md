...
| **GPU 1 (Sub)** | **RTX 3090** | **24GB** | **Vision AI**, **TTS** (GPT-SoVITS), **STT**, **3D Rendering** | **3060에서 업그레이드 확정**. 넉넉한 VRAM으로 Vision AI, 고품질 TTS 및 렌더링 전담. |
...
-   **할당 전략**:
    -   **GPU 0 (RTX 5080)**: Main LLM (Gemma 3 27B 4-bit) 전용
...
*   **Quantization**: **4-bit GGUF/EXL2**.
    ...
    *   *특징*: Gemma 3는 멀티모달(Vision) 능력이 강화되어, 별도의 Vision 모델 없이도 이미지 이해 가능성이 높음. 다만 Vision AI 처리는 RTX 3090에서 수행하여 RTX 5080의 VRAM 부담을 완화.
*   **Context Window**: 4k ~ 8k (VRAM 잔여량에 따라 가변적). Vision AI를 RTX 3090으로 이동하여 더 긴 컨텍스트 사용 가능.
